...
def main():
    ...
    pathToMain = os.path.join(args.input_dir, "_main.pd")

    # for temporarily removing the conv~ wrapper abstraction
    pathToConv = os.path.join(args.input_dir, "abs/conv~.pd")
    pathToConvTmp = os.path.join(args.input_dir, "abs/conv-tmp.pd")
    ...
    try:
        ...
        if os.path.exists(pathToConv):
            shutil.move(pathToConv, pathToConvTmp)
        zip_path = __zip_dir(
            args.input_dir,
    ...
    # putting the conv~ abstraction back in it's original place
    if os.path.exists(pathToConvTmp):
        shutil.move(pathToConvTmp, pathToConv)
